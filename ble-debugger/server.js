...
const { asError, createBleDebuggerCore } = require('./core');
...
function i18nPathFromRequest(requestPath) {
  const match = requestPath.match(/^\/(?:tools\/ble-debugger\/)?i18n\/([a-zA-Z0-9_-]+\.json)$/);
  if (!match) return '';
...
async function startBleDebuggerServer(options = {}) {
  ...
  const core = createBleDebuggerCore({
  ...
  async function stop() {
    ...
    for (const client of clients) {
      client.close(1001, 'BLE debugger server closed');
    }
  ...
  wss.on('connection', socket => {
    clients.add(socket);
    const currentStatus = core.status();
    socket.send(JSON.stringify({
      event: 'ready',
      data: {
        state: currentStatus.state,
        scanning: currentStatus.scanning,
        connectedDevices: currentStatus.connectedDevices || [],
        pid: process.pid
      }
    }));
