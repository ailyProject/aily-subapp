...
<tool-id>/
  ...
  ui/
    index.html
    ... framework bundles or UI-bundled assets
  vendor/
    penpal.min.js
...
每个子应用自己的业务静态资源必须放在该子应用目录下的 `assets/` 中，例如字库文本、示例数据、WASM 辅助数据、模板数据等。UI 通过 `/assets/<path>` 或宿主前缀形式 `/tools/<tool-id>/assets/<path>` 加载。不要把这类资源放到仓库根目录、公共临时目录或其它子应用目录下。`ui/` 内仍然可以保留由前端构建器生成、且只被 `ui/index.html` 引用的图片、CSS chunk 等 UI bundle 资源。
...
- `assets/`：本工具专属业务静态资源，由 child server 提供给 UI 读取，不与其它子应用共享路径。
...
assets 要求：
- 业务静态资源放在 `assets/<path>`。
- child server 必须能提供：
  `/assets/<path>`
  `/tools/<tool-id>/assets/<path>`
- `assets/` 会随构建产物一起复制，适合放稳定的小到中型静态资源；临时文件、用户下载文件和运行期缓存不要放在这里。
...
<tool-id>/
  ...
  ui/
    index.html
    ... compiled JS/CSS/UI assets or static UI files
  vendor/
    penpal.min.js
  skill/
    <optional-skill-name>/
...
最小可运行目录可以只有 `en.json` 和必要 JS/CSS，但正式交付建议补齐主应用已有语言集合。工具需要的固定业务资源统一放在 `assets/`；如果没有这类资源，可以不创建该目录。
...
`index.html` 可以引用任意构建产物。对于 Angular、Vue、React、Vite 等项目，JS/CSS 文件名可以由构建器生成，UI bundle 自己引用的资源也可以放在 `ui/assets/`、`ui/static/` 或其他由 `index.html` 正确引用的目录中。与工具业务有关、需要由代码按路径读取的静态资源放在子应用根目录的 `assets/`。
...
- `/i18n/zh_cn.json` 可访问。
- 如果工具有 `assets/`，`/assets/<path>` 可访问，`/tools/<tool-id>/assets/<path>` 也可访问。
