...
`{{Tool Name}}` is an Angular child application template for Aily Blockly / Aily Chat AI IDE. Copy this directory, replace the placeholders, and then put the real tool behavior in `core.js`, `cli.js`, `server.js`, and `ui/src/app/`.
...
- `server.js`: serves the built Angular UI from `dist/{{tool-id}}/ui` in source mode, falls back to packaged `ui/`, serves `i18n/` and `assets/`, exposes `/ws`, validates `token`, and handles `/health` plus `/api/shutdown`.
