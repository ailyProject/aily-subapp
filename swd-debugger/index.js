...
const { startSwdDebuggerServer } = require('./server');
...
function startRpcServer() {
  ...
  try {
    core = createSwdDebuggerCore({ sendEvent });
  } catch (error) {
...
async function startServeMode(rawArgs) {
  ...
  try {
    serverHandle = await startSwdDebuggerServer({
...
if (requestedCommand === 'rpc' || requestedCommand === 'server') {
  ...
  void runCli(requestedCommand, cliArgs.slice(1), {
    createCore: () => createSwdDebuggerCore(),
