...
const { asError, createSwdDebuggerCore } = require('./core');

const TOOL_ID = 'swd-debugger';
const MIME_TYPES = {
...
function fontPathFromRequest(requestPath) {
  ...
  let decoded = '';
  try {
    decoded = decodeURIComponent(requestPath.replace(/^\/fonts\//, ''));
  } catch {
    return '';
  }
  if (decoded.includes('\0')) return '';
...
function requestUrlFrom(request, host) {
  try {
    return new URL(request.url || '/', `http://${request.headers.host || host || '127.0.0.1'}`);
  } catch {
    return null;
  }
}
...
async function startSwdDebuggerServer(options = {}) {
  ...
  const core = createSwdDebuggerCore({
  ...
  async function stop() {
    ...
    for (const client of clients) client.close(1001, 'SWD debugger server closed');
  ...
  server = http.createServer((request, response) => {
    const requestUrl = requestUrlFrom(request, host);
    if (!requestUrl) {
      response.writeHead(400);
      response.end('Bad Request');
      return;
  ...
  server.on('upgrade', (request, socket, head) => {
    const requestUrl = requestUrlFrom(request, host);
    if (!requestUrl || requestUrl.pathname !== '/ws' || !verifyToken(requestUrl, token)) {
