...
const { asError, createIndustrialBusDebuggerCore } = require('./core');

const TOOL_ID = 'industrial-bus-debugger';
const MIME_TYPES = {
...
async function startIndustrialBusDebuggerServer(options = {}) {
  ...
  const core = createIndustrialBusDebuggerCore();
  ...
  async function stop() {
    ...
    for (const client of clients) {
      client.close(1001, 'Industrial bus debugger server closed');
    }
...
module.exports = {
  startIndustrialBusDebuggerServer
};
