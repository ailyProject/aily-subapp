...
当只开发 child 子应用本身时，不需要启动 Angular/Electron 主应用。可以在 `aily-tools` 仓库根目录直接启动单个工具：

```bash
npm run dev -- <tool-id>
```

首次运行某个工具前，如果该工具目录还没有安装依赖，先执行：
...
npm run dev:network
npm run dev:ffs
npm run dev -- ffs-manager-child
```

`dev` 命令由 `scripts/dev-tool.mjs` 提供，内部仍然调用工具自己的 `node index.js serve`，但会额外做三件事：

- 使用稳定端口和稳定 token 启动子服务，后端重启后浏览器地址仍然可用。
- 给 HTML 注入仅开发态启用的 live reload 脚本。
- 监听工具目录变化，按变更类型自动刷新或重启。

默认行为：

- `ui/` 和 `i18n/` 下文件变化：不重启 Node 后台，只通知浏览器刷新。
- 根部 `index.js`、`server.js`、`core.js`、`cli.js`、`package.json`、`package-lock.json` 变化：自动重启子服务，ready 后再通知浏览器刷新。
- 默认追加 `lang=zh_cn`、`theme=dark`，用于模拟主应用 iframe 首次加载时传入的上下文。
- 默认只监听 `127.0.0.1`。

可选参数：

```bash
npm run dev -- <tool-id> --host 127.0.0.1 --port 18450 --reload-port 19450
npm run dev -- <tool-id> --lang en --theme light
npm run dev -- <tool-id> --open
```

独立开发模式的边界：

- 它只模拟 iframe URL 上的 `lang`、`theme` 和本地 HTTP/WebSocket 环境，不启动主应用。
- child UI 必须能在没有父页面 Penpal host 的情况下完成基础渲染和后端连接。
- 需要验证宿主能力时，例如 `requestClose()`、`requestRestart()`、`openExternal()`、主应用主题/语言实时同步，仍需要回到主应用内验证。
