...
const { asError, createNetworkDebuggerCore } = require('./core');

const TOOL_ID = 'network-debugger';
const MIME_TYPES = {
...
async function startNetworkDebuggerServer(options = {}) {
  ...
  const core = createNetworkDebuggerCore({
  ...
  async function stop() {
    ...
    for (const client of clients) {
      client.close(1001, 'Network debugger server closed');
    }
