...
function helpText() {
  return [
    'Aily U8g2 Font Generator CLI',
    '',
    'Usage:',
    '  node index.js rpc',
    '  node index.js serve [--host 127.0.0.1] [--port 0]',
    '  node index.js status',
    '  node index.js fonts [--refresh]',
    '',
    'The UI performs glyph rasterization in the browser canvas. CLI commands write one JSON object to stdout.'
  ].join('\n');
}
  ...
  try {
    ...
    switch (command) {
      ...
      default:
        if (typeof createCore !== 'function') {
          throw new Error('U8g2 font generator core factory is not available');
        }
        core = createCore();
    }
    ...
    switch (command) {
      case 'status':
        ...
        break;
      case 'fonts':
        data = core.listFonts({ refresh: Boolean(options.refresh) });
        break;
      default:
