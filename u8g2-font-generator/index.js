...
const { startU8g2FontGeneratorServer } = require('./server');
...
function startRpcServer() {
  ...
  try {
    core = createU8g2FontGeneratorCore({ sendEvent });
  } catch (error) {
...
async function startServeMode(rawArgs) {
  ...
  try {
    serverHandle = await startU8g2FontGeneratorServer({
...
if (requestedCommand === 'rpc' || requestedCommand === 'server') {
  ...
  void runCli(requestedCommand, cliArgs.slice(1), {
    createCore: () => createU8g2FontGeneratorCore(),
