...
const { asError, createVideoConverterCore } = require('./core');
const { parseCliArgs, requestedCommandFromArgs, runCli } = require('./cli');
const { startVideoConverterServer } = require('./server');
...
function startRpcServer() {
  ...
  try {
    core = createVideoConverterCore({ sendEvent });
  } catch (error) {
...
async function startServeMode(rawArgs) {
  ...
  try {
    serverHandle = await startVideoConverterServer({
...
if (requestedCommand === 'rpc' || requestedCommand === 'server') {
  ...
  void runCli(requestedCommand, cliArgs.slice(1), {
    createCore: () => createVideoConverterCore(),
